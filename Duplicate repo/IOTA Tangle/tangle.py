...
class Tangle(object):
   ...
   # used to make sure that the pow was correct
   @staticmethod
   ...
   # create a new node
   # these prevNodes are the indexes in the dag that points to the previous nodes
   def createNode(self, data, prevNodes, newIndex, validity=0):
   ...
   def send_transaction(self, data):
      ...
      for i in range(len(self.nodes)-1, -1, -1):
         ...
         if node['validity'] < prop.RequiredProofs:
            ...
            if worstCaseScenario == [] or len(worstCaseScenario) < prop.numberOfValidationNodesNeeded:
               worstCaseScenario.append(node)
               worstCaseScenarioindexes.append(node['index'])
      ...
      while len(nodesToattach) < prop.numberOfValidationNodesNeeded:
         ...
         nodesToattach.append(worstCaseScenario.pop())
         nodesIndexes.append(worstCaseScenarioindexes.pop())
...
###########################################################################
   ...
   # we will use this to ensure that the tangles we get from peers are correct
   ...
   # we will use this a add new peers to our p2p network
